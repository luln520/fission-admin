...
//            BigDecimal ploss = twLeverOrder.getPloss().subtract(twLeverOrder.getPremium());
            ...
            //买涨
              if(hyzd == 1){
                    if(kongyk == 1){  //盈利
                        BigDecimal ploss =  twLeverOrder.getNum().multiply(new BigDecimal(twLeverOrder.getWin()).divide(new BigDecimal(100)),mathContext);  //盈利金额
                    ...
                    if(kongyk == 2){ //亏损
                        BigDecimal ploss =  twLeverOrder.getNum().multiply(new BigDecimal(twLeverOrder.getLoss()).divide(new BigDecimal(100)),mathContext); //亏损金额
                        if (buyprice.compareTo(newprice) < 0) {
