...
//        twLeverOrder.setIntselltime((int) (selltime.getTime()/1000));
    ...
    public ResponseDTO creatorderNew(int uid, String ccoinname, int win, int loss, int fold, int hyzd, BigDecimal num, BigDecimal ploss, BigDecimal premium, String language, BigDecimal lossPrice, BigDecimal winPrice, BigDecimal boomPrice) {
        ...
        if(userType == 1){   //会员订单
           return  userOrder(twUser,language,  ccoinname,  win,  loss,  fold,  hyzd,  num,  ploss,  lossPrice,  winPrice, boomPrice);
        }

        if(userType == 2){   //模拟订单
           return  mockUserOrder(twUser,language,  ccoinname,  win,  loss,  fold,  hyzd,  num,  ploss,  lossPrice,  winPrice, boomPrice);
        }

        return null;
    }

    public ResponseDTO userOrder(TwUser twUser,String language,String ccoinname, int win, int loss, int fold, int hyzd, BigDecimal num, BigDecimal ploss,  BigDecimal lossPrice, BigDecimal winPrice, BigDecimal boomPrice) {
...
//        twLeverOrder.setIntselltime((int) (selltime.getTime()/1000));
    ...
    public ResponseDTO mockUserOrder(TwUser twUser,String language,String ccoinname, int win, int loss, int fold, int hyzd, BigDecimal num, BigDecimal ploss,  BigDecimal lossPrice, BigDecimal winPrice, BigDecimal boomPrice) {
        Integer uid = twUser.getId();
        TwCompany company = twCompanyService.getById(twUser.getCompanyId());
        int inviteType = company.getInviteType();
        String invite = "";
...
//        TwLeverage one = twLeverageService.getOne(queryWrapper3);
        ...
        BigDecimal leverFee = company.getLeverFee();
        BigDecimal nums = num.add(leverFee);
        if(twMockUserCoin.getUsdt().compareTo(nums) < 0){
            if(language.equals("zh")){
                return ResponseDTO.userErrorParam("余额不足！");
            }else{
                return ResponseDTO.userErrorParam("Insufficient balance！");
        ...
        twLeverOrder.setHyzd(hyzd);
        twLeverOrder.setCompanyId(twUser.getCompanyId());
        twLeverOrder.setBuyOrblance(twMockUserCoin.getUsdt().subtract(nums));
        twLeverOrder.setCoinname(ccoinname);
        twLeverOrder.setStatus(1);
        twLeverOrder.setIsWin(0);
        twLeverOrder.setOrderType(2);
        twLeverOrder.setLossPrice(lossPrice);
        twLeverOrder.setLoss(loss);
        twLeverOrder.setWin(win);
        twLeverOrder.setWinPrice(winPrice);
...
//        twLeverOrder.setIntselltime((int) (selltime.getTime()/1000));
        ...
        this.save(twLeverOrder);
        //扣除USDT额度
        twMockUserCoinService.decre(uid,nums,twMockUserCoin.getUsdt());
