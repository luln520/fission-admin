...
class IssueController extends HomeController
{
	...
	public function normalissue(){
		...
		$uid = userid();
		$list = M("issue_log")->where(array('uid'=>$uid,'O.status'=>1))->order("O.id desc")->field('*,O.num as onum')->alias('as O') //给表取个别名  ID逆序必须指定是哪个表 是o  还是  W
        ->join('LEFT JOIN `' . C('DB_PREFIX') . 'issue` AS W ON W.id = O.pid')->select();
	...
	public function upbuynum($pid=null,$num=null){
		...
		$uid = userid();
		$uinfo = M("user")->where(array('id'=>$uid))->field("id,rzstatus,username,invit1,invit2,invit3")->find();
		if(empty($uinfo)){
        ...
        if($logre && $upre && $decre && $decbillre && $incre && $incbillre){
            ...
            if($uinfo['invit1'] > 0){
                ...
                if($onemoney > 0){
                    $this->sendteamjl($uinfo['invit1'],$onemoney,$jlcoin,1);
                }
            }
            if($uinfo['invit2'] > 0){
                $twobl = $issue['two_jl'];
                $twomoney = $num * $twobl / 100;
                $this->sendteamjl($uinfo['invit2'],$twomoney,$jlcoin,2);
            }
            if($uinfo['invit3'] > 0){
                $threebl = $issue['three_jl'];
                $threemoney = $num * $threebl / 100;
                $this->sendteamjl($uinfo['invit3'],$threemoney,$jlcoin,3);
            }
