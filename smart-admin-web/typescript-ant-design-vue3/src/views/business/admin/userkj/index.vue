    ...
    <a-table :columns="columns" :data-source="tableData" @change="tableChange" :scroll="{ x: 1500 }"
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'imgs'">
          <a-image height="60px" :preview="true" :src="imageConfig.baseImageUrl +record.imgs"
...
async function updateStatus(id, type) {
  let data = await kuangmApi.userkjStatus({ id, type });
  if (data.ok) {
...
async function deleteData(id) {
  let data = await kuangmApi.userkjStatus({ id, type: 3 });
  if (data.ok) {
    message.success("操作成功！")
  } else {
    message.error("操作失败！请重试~")
...
async function addOrEditSubmit(submitData) {
  let data = await kuangmApi.addOrUpdate(submitData);
  if (data.ok) {
    message.success("操作成功！");
