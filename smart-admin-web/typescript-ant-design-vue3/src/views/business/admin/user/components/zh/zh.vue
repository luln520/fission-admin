<template>
    <a-modal width="800px" :destroyOnClose="true" :visible="props.isOpen" :open="props.isOpen" title="账号控制"
        @ok="addHandleOk" @cancel="close">
        <a-form ref="dataFormRef" :label-col="labelCol" :model="dataModel">
            <a-form-item :name="'x'" :label="'账号控制'">
                <a-radio-group v-model:value="dataModel.status" :options="optionsWithDisabled" />
            </a-form-item>
        </a-form>
    </a-modal>
</template>
