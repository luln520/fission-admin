<template>
    ...
    <a-table :columns="columns" :data-source="tableData" @change="tableChange" :scroll="{ x: 1500 }"
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'type'">
          <span v-if="record.type != 1" style="color: green;"> 止盈</span>
          <span v-if="record.type == 1" style="color: red;">止损</span>
        </template>
  ...
  <AddOrEdit v-if="isOpenEdit" ref="addOrEditRef" :isOpen="isOpenEdit" :formItems="formItems" :type="addOrEditType"
    :name="'止盈止损设置'" :data="addOrEditData" @close="() => {
      isOpenEdit = false;
...
import { leversetApi } from '/@/api/business/admin/leverset-api';
...
async function deleteData(id) {
  let data = await leversetApi.delete({ id });
  if (data.ok) {
...
async function updateStatus(id, status) {
  let data = await leversetApi.updateStatus({ id, status });
  if (data.ok) {
    message.success("操作成功！")
  } else {
    message.error("操作失败！请重试~")
...
async function addOrEditSubmit(submitData) {
  let data = await leversetApi.addOrUpdate(submitData);
  if (data.ok) {
    message.success("操作成功！");
...
async function loadData() {
  let data = await leversetApi.list({ pageNum: pagination.value.current, pageSize: pagination.value.pageSize });
  if (data.ok) {
    data = data.data;
