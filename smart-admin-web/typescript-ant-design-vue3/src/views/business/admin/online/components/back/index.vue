<template>
  <a-modal width="800px" :destroyOnClose="true" :visible="props.isOpen" :open="props.isOpen" :maskClosable="false"
    ...
    <!-- 聊天框 -->
    <div id="scrollDiv" style="height: 300px;overflow-y: scroll;border: #d8d8d8 2px solid;padding: 5px;">
      <div v-for="(item, index) in msgData" :key="index">
        <div>
          <div v-if="item.type == 1"
            style="background-color: #d8d8d8; border-radius: 5px;width: 70%;float: right;margin-bottom: 10px;padding:5px ;">
            {{ item.content }}</div>
          <div v-if="item.type == 2"
            style="background-color: #d8d8d8; border-radius: 5px;width: 70%;float: left;margin-bottom: 10px;padding:5px">
            {{ item.content }}</div>
        </div>
      </div>
    </div>
