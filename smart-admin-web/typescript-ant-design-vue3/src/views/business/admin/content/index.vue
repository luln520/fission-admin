...
import { contentApi } from '/@/api/business/admin/content-api';
...
async function deleteData(id) {
  let data = await contentApi.delete({ id });
  if (data.ok) {
...
async function updateStatus(id, status) {
  let data = await contentApi.updateStatus({ id, status });
  if (data.ok) {
    message.success("操作成功！")
  } else {
    message.error("操作失败！请重试~")
...
async function addOrEditSubmit(submitData) {
  let data = await contentApi.addOrUpdate(submitData);
  if (data.ok) {
    message.success("操作成功！");
...
async function loadData() {
  let data = await contentApi.list({ pageNum: pagination.value.current, pageSize: pagination.value.pageSize });
  if (data.ok) {
    data = data.data;
