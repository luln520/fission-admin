<template>
    ...
    <a-table :columns="columns" :data-source="tableData" @change="tableChange" :scroll="{ x: 1500 }"
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'payimg'">
          <a-image height="60px" :preview="true" :src="imageConfig.baseImageUrl + record.payimg"
  ...
  <!-- 编辑弹框 -->
  <AddOrEdit v-if="isOpenEdit" ref="addOrEditRef" :isOpen="isOpenEdit" :formItems="formItems" :type="addOrEditType" width="400px"
    :name="'金额'" :data="addOrEditData" @close="() => {
      isOpenEdit = false;
...
async function addOrEditSubmit(submitData) {
  let data = await financeApi.rechargeNum(submitData);
  if (data.ok) {
    message.success("操作成功！");
